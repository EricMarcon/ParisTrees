...
CRANLibrary(c("kableExtra", "rgdal", "tidyverse", "entropart"))
...
Exportation du jeu de données entier au format GeoJSON dans `/data/les-arbres.les-arbres.geojson`.
...
# Vérification de l'encodage correct des acccents
levels(les_arbres@data$libellefrancais)
# Projection en Lambert 93 pour les coordonnées métriques
...
les_arbres@data %>% 
  ...
  summarise(Nombre=length(domanialite)) %>% 
  arrange(desc(Nombre))
```
...
les_arbres@data %>% 
  ...
  summarise(Nombre=length(arrondissement)) %>% 
  arrange(desc(Nombre))
```
...
les_arbres_df %>% 
  group_by(spName) %>% 
  summarise(Nombre=length(spName)) %>% 
  arrange(desc(Nombre)) ->
  AbdFreqCount
```
...
```{r, tidy=FALSE}
library("dbmss")
# Extraction d'un wmppp à partir d'une adresse
wmppp_adresse <- function(address) {
  ...
  les_arbres_df %>% 
    ...
    Points
  # Fenêtre = polygone convexe autour des points (mieux qu'un rectangle)
...
```{r}
Buttes_Chaumont <- wmppp_adresse("PARC DES BUTTES CHAUMONT")
