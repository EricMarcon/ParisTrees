...
- Explore the biodiversity of trees,
...
Dataset: 200,000 trees in Paris located in 20 arrondissements, excluding Bois de Vincenne and Bois de Boulogne.
...
Reproducible research: code and data available on https://github.com/EricMarcon/ParisTrees.
...
"A tree of arrondissement $i$ belongs to the species" 
...
Plane tree, horse chestnut, scholar tree, lime tree, Norway maple, sycamore maple...
...
Key importance of the order of diversity/ubiquity: Theil$\leftrightarrow q=1$; Ellison and Glaeser$\leftrightarrow q=2$.
...
- Spatial concentration metrics are available [@Marcon2012c],

- Spatially explicit diversity metrics to develop.

```{r, message=FALSE}
Buttes_Chaumont <- wmppp_adresse("PARC DES BUTTES CHAUMONT 19 Public garden")
plot(Buttes_Chaumont, which.marks="PointWeight", main="Parc des Buttes Chaumont")
```
...
```{r, message=FALSE}
Buttes_Chaumont_accum <- DivAccum(Buttes_Chaumont, q.seq=c(0,2), r.seq=c(0,30), Individual=TRUE)
par(mar=rep(0,4))
capture.output(MapPlot(Buttes_Chaumont_accum, Order=0, NeighborHood=30)) -> silence
```
